      ...
      externalConstraint("DateValidator", (value, key, { config, record }) => {
        switch(config.type) {
          ...
          case ValidationType.EVALUATE_AND_FORMAT:
            DateValidator.evaluateAndFormat(record, key, config.validationType, config.format, config.validationArgs, config.options);
            break;
        }
      })
    );
      ...
      externalConstraint("NumberValidator", (value, key, { config, record }) => {
        switch(config.type) {
          ...
          case ValidationType.PRECISION:
            ...
            break;
          case ValidationType.EVALUATE_AND_FORMAT:
            NumberValidator.evaluateAndFormat(record, key, config.validationType, config.formatOptions, config.validationArgs, config.options);
            break;
        }
      })
    );
      ...
      externalConstraint("StringValidator", (value, key, { config, record }) => {
        switch(config.type) {
          ...
          case ValidationType.FORMAT_PHONE:
            ...
            break;
          case ValidationType.EVALUATE_AND_FORMAT:
            StringValidator.evaluateAndFormat(record, key, config.validationType, config.formatOptions, config.validationArgs, config.options);
            break; 
